import 'package:flutter/material.dart';
import 'package:flutter_screenutil/flutter_screenutil.dart';

abstract class AppStyles {
  static String fontFamily = 'Poppins';

  static TextStyle textRegular10 = TextStyle(
    fontWeight: FontWeight.w400,
    fontSize: 10.sp,
  );
  static TextStyle textRegular12 = TextStyle(
    fontWeight: FontWeight.w400,
    fontSize: 12.sp,
  );
  static TextStyle textRegular14 = TextStyle(
    fontWeight: FontWeight.w400,
    fontSize: 14.sp,
  );
  static TextStyle textRegular16 = TextStyle(
    fontWeight: FontWeight.w400,
    fontSize: 16.sp,
  );
  static TextStyle textRegular18 = TextStyle(
    fontWeight: FontWeight.w400,
    fontSize: 18.sp,
  );
  static TextStyle textRegular20 = TextStyle(
    fontWeight: FontWeight.w400,
    fontSize: 20.sp,
  );
  static TextStyle textRegular22 = TextStyle(
    fontWeight: FontWeight.w400,
    fontSize: 22.sp,
  );
  static TextStyle textRegular24 = TextStyle(
    fontWeight: FontWeight.w400,
    fontSize: 24.sp,
  );

  static TextStyle textMedium12 = TextStyle(
    fontWeight: FontWeight.w500,
    fontSize: 12.sp,
  );
  static TextStyle textMedium14 = TextStyle(
    fontWeight: FontWeight.w500,
    fontSize: 14.sp,
  );
  static TextStyle textMedium16 = TextStyle(
    fontWeight: FontWeight.w500,
    fontSize: 16.sp,
  );
  static TextStyle textMedium18 = TextStyle(
    fontWeight: FontWeight.w500,
    fontSize: 18.sp,
  );
  static TextStyle textMedium20 = TextStyle(
    fontWeight: FontWeight.w500,
    fontSize: 20.sp,
  );
  static TextStyle textMedium22 = TextStyle(
    fontWeight: FontWeight.w500,
    fontSize: 22.sp,
  );
  static TextStyle textMedium24 = TextStyle(
    fontWeight: FontWeight.w500,
    fontSize: 24.sp,
  );

  static TextStyle textSemiBold12 = TextStyle(
    fontWeight: FontWeight.w600,
    fontSize: 12.sp,
  );
  static TextStyle textSemiBold14 = TextStyle(
    fontWeight: FontWeight.w600,
    fontSize: 14.sp,
  );
  static TextStyle textSemiBold16 = TextStyle(
    fontWeight: FontWeight.w600,
    fontSize: 16.sp,
  );
  static TextStyle textSemiBold18 = TextStyle(
    fontWeight: FontWeight.w600,
    fontSize: 18.sp,
  );
  static TextStyle textSemiBold20 = TextStyle(
    fontWeight: FontWeight.w600,
    fontSize: 20.sp,
  );
  static TextStyle textSemiBold22 = TextStyle(
    fontWeight: FontWeight.w600,
    fontSize: 22.sp,
  );
  static TextStyle textSemiBold24 = TextStyle(
    fontWeight: FontWeight.w600,
    fontSize: 24.sp,
  );

  static TextStyle textBold12 = TextStyle(
    fontWeight: FontWeight.w700,
    fontSize: 12.sp,
  );
  static TextStyle textBold14 = TextStyle(
    fontWeight: FontWeight.w700,
    fontSize: 14.sp,
  );
  static TextStyle textBold16 = TextStyle(
    fontWeight: FontWeight.w700,
    fontSize: 16.sp,
  );
  static TextStyle textBold18 = TextStyle(
    fontWeight: FontWeight.w700,
    fontSize: 18.sp,
  );
  static TextStyle textBold20 = TextStyle(
    fontWeight: FontWeight.w700,
    fontSize: 20.sp,
  );
  static TextStyle textBold22 = TextStyle(
    fontWeight: FontWeight.w700,
    fontSize: 22.sp,
  );
  static TextStyle textBold24 = TextStyle(
    fontWeight: FontWeight.w700,
    fontSize: 24.sp,
  );
  static TextStyle textBold32 = TextStyle(
    fontWeight: FontWeight.w700,
    fontSize: 32.sp,
  );

  static TextStyle textExtraBold18 = TextStyle(
    fontWeight: FontWeight.w800,
    fontSize: 18.sp,
  );
  static TextStyle textExtraBold24 = TextStyle(
    fontWeight: FontWeight.w800,
    fontSize: 24.sp,
  );
  static TextStyle textExtraBold30 = TextStyle(
    fontWeight: FontWeight.w800,
    fontSize: 30.sp,
  );
  static TextStyle textExtraBold32 = TextStyle(
    fontWeight: FontWeight.w800,
    fontSize: 32.sp,
  );
  static TextStyle textExtraBold36 = TextStyle(
    fontWeight: FontWeight.w800,
    fontSize: 36.sp,
  );
}
